...
②▼ご提案したいキャリアの可能性（箇条書き2〜3点）【セクション全体で250〜300文字以内。超過禁止】
各項目の書き方：
「・【具体的な職種名】：現職の『○○スキル』を抽象化すると『△△力』として、〜〜の場面で活躍できる可能性」
ポイント：1項目あたり60〜80文字に収める。候補者の経歴を長く引用・解説しない。スキルの橋渡しを一言で端的に表現する
例）「・【法人営業職】：秘書業務で培った"先回り力"を交渉・提案力として活かせる可能性」
候補者の「興味のある働き方」（社会貢献・WLB・グローバルなど）は最大1点だけ盛り込む
...
④▼今回のご転職活動のポイント（■ポイント①②）【セクション全体で500文字以内。超過禁止】
各ポイントは「候補者固有のリスク1文＋PARQUEの介在価値1文」の計2文で完結させる。一般的なアドバイス・PARQUEのサービス説明の列挙は禁止。シャープに絞ること。

「■ポイント①：【ポイントを一言で表すタイトル】」
・候補者固有の面接失敗パターンを具体的に1文で描写（例：「面接でホスピタリティばかり強調し、目標達成意欲が伝わらずにお見送りになるケース」）
・PARQUEの介在価値を1文で端的に示す

「■ポイント②：【ポイントを一言で表すタイトル】」
・中長期キャリアの懸念を1文で描写（横滑り転職のリスク・志向性とのミスマッチなど）
・5年後・10年後を見据えたロードマップ提供の介在価値を1文で端的に示す
...
- 視認性のため、①②③④の各セクション間・▼見出しの前後・■ポイントの前後には1行の空白行を入れる。ただし同一セクション内の文章間は空白行を入れず、句点後の改行のみとする。3行以上の連続空白行は禁止。
- 冒頭に自己紹介文（「はじめまして、PARQUEの〇〇です」「はじめてご連絡します」などの書き出し）は不要。プロフィールへの言及から直接始める。
- 末尾に「お気軽にご返信ください」「ご連絡をお待ちしております」「カジュアル面談」「30分程度」などの締め文は一切書かない。テンプレート側で補完されるため本文では不要。
...
【文字数制限（最重要・絶対厳守）】
- 本文全体（①〜④）を合計2,000文字以内に収めること
- ②「ご提案したいキャリアの可能性」：250〜300文字以内
- ④「今回のご転職活動のポイント」：500文字以内
- いずれかが超過しそうな場合は削って調整する。文字数を理由に内容を薄めるのではなく、同じ情報をより短い言葉で表現する
